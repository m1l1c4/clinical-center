...
public class ClinicControllerTest {
	...
	/*
	 * @Before(value = "") public void login() { ResponseEntity<UserLoggedInDTO>
	 * responseEntity = restTemplate.postForEntity("/login", new
	 * JwtAuthenticationRequest("pacijent@gmail.com", "sifra1"),
	 * UserLoggedInDTO.class); accessToken = "Bearer " +
	 * responseEntity.getBody().getUserTokenState().getAccessToken(); }
	 */
	...
	public void testSearchClinicsControllerOk() throws Exception {
		/*
		 * Clinic clinic1 = new Clinic(ClinicConstants.ID_C_1, ClinicConstants.NAZIV_1,
		 * ClinicConstants.GRAD_1, ClinicConstants.DRZAVA_1, ClinicConstants.ADRESA_1,
		 * ClinicConstants.RAITING_1, ClinicConstants.OPIS_1);
		 * 
		 * Clinic clinic2 = new Clinic(ClinicConstants.ID_C_2, ClinicConstants.NAZIV_2,
		 * ClinicConstants.GRAD_1, ClinicConstants.DRZAVA_1, ClinicConstants.ADRESA_1,
		 * ClinicConstants.RAITING_1, ClinicConstants.OPIS_1);
		 * 
		 * List<ClinicDTO> clinics = new ArrayList<ClinicDTO>(); ClinicDTO clinic11 =
		 * new ClinicDTO(clinic1); ClinicDTO clinic12 = new ClinicDTO(clinic2);
		 * clinics.add(clinic11); clinics.add(clinic12); clinic1.setMedicalStuff(new
		 * HashSet<MedicalWorker>()); clinic2.setMedicalStuff(new
		 * HashSet<MedicalWorker>());
		 * 
		 * User user1 = new User(); user1.setName(UserConstants.IME_1);
		 * user1.setSurname(UserConstants.PREZIME_1); user1.setType(UserConstants.TIP);
		 * 
		 * User user2 = new User(); user2.setName(UserConstants.IME_2);
		 * user2.setSurname(UserConstants.PREZIME_2); user2.setType(UserConstants.TIP);
		 * 
		 * MedicalWorker mw1 = new MedicalWorker(DoctorConstants.DOCTOR_ID_1, user1,
		 * clinic1, DoctorConstants.TIP_D_1); clinic1.getMedicalStuff().add(mw1);
		 * 
		 * MedicalWorker mw2 = new MedicalWorker(DoctorConstants.DOCTOR_ID_2, user2,
		 * clinic2, DoctorConstants.TIP_D_1); clinic2.getMedicalStuff().add(mw2);
		 * 
		 * CheckUpType srchType = new CheckUpType(); srchType.setClinics(new
		 * HashSet<Clinic>()); srchType.getClinics().add(clinic1);
		 * srchType.getClinics().add(clinic2);
		 * srchType.setName(CheckupTypeConstants.CHECK_UP_TYPE_NAME);
		 * srchType.setId(CheckupTypeConstants.CHECK_UP_TYPE_ID);
		 * srchType.setTypePrice(100);
		 * 
		 * Mockito.when(checkupTypeRepository.findOneByName(CheckupTypeConstants.
		 * CHECK_UP_TYPE_NAME)).thenReturn(srchType);
		 */
